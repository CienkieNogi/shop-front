...
const Test = () => {
  ...
  const [getProductByName, { data: productsByName }] =
    useGetProductByNameMutation();
  const [getProductByCategory, { data: productCategory, isLoading }] =
    useGetProductByCategoryMutation();
  const [createProduct] = useCreateProductMutation();
  const [deleteProduct] = useDeleteProductMutation();
console.log({isLoading});
  const handleSearch = async () => {
    await getProductByCategory({category:search,page:1});
  };
  ...
  return (
    <div className="testcont">
      <div className="test">
        {products?.map((el) => (
          <div key={el.id} className="prod">
            ...
            <p className="cosik" onClick={() => handleDelete(el)}>
              X
            </p>
          </div>
        ))}
      ...
      {isLoading && <Spinner />}
      {!isLoading && productCategory?.map((el) => <p>{el.name}</p>)}
