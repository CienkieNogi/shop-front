...
export const extendedApiSlice = api.injectEndpoints({
    endpoints: (builder) => ({
        getProducts: builder.query<ServerResponseWithCount<ProductI[]>, string>({
            query: (id) => `/product/?categoryId=${id}`,
        ...
        getSuggestedProducts: builder.mutation<ServerResponse<ProductI[]>, { category: string }>({
            query: ({ category }) => ({
...
export const {
    ...
    useGetSuggestedProductsMutation
} = extendedApiSlice;
